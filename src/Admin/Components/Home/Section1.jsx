  ...
  useEffect(() => {
    const fetchGraphData = async () => {
      try {
        ...
        setPieData(data.pieData || [{ name: "No Data", value: 0 }]); // Ensures empty pie chart
        setRevenueData(data.revenueData || []); // Ensures empty line chart
  ...
  const renderLabel = ({ cx, cy }) => {
    return (
      <text x={cx} y={cy} textAnchor="middle" dominantBaseline="middle" fontSize="18px" fontWeight="bold" fill="#2CA35C">
        {pieData.length > 0 && pieData[0].value ? `${pieData[0].value}%` : ""}
      </text>
    );
  };
